...
钱包APP未对于hook框架检测，会导致APP运行的时候被hook框架拦截分析函数和流程，使得APP相关核心执行过程被逆向调试分析。

- 钱包APP需要做运行环境是否存在hook框架（包括xposed、frida、substrate等）检查，如果检测到运行环境存在hook框架，APP提示用户并且强制退出。
...
- 助记词长度为12-24，一般采用12。
- 助记词库必须足够大来防止被暴力破解，目前的多款钱包的助记词库都为2048，根据数学的排列组合公式： n!/( n - r )! ，既 2048!/(2048-12)! = 5.2715379713014884760003093175282e+39，说明已经足够强大。
- 助记词产生的随机数必须采用安全随机数，例如使用java的SecureRandom而不是Random。
- iBitcome钱包生成助记词的词库必须足够强壮和完备。
...
- 使用助记词生成Seed种子。
- 使用助记词生成的Seed种子字节数组的0-31字节作为私钥。
- 使用助记词作为输入生成的Seed生成公私钥对ECKeyPair。
- 采用secp256k1椭圆曲线作为椭圆曲线算法的输入生成ECKeyPair。
...
- 使用生成的Keypair和授权密码作为输入，采用不可逆算法生成keystore文件。
...
- keystore文件存储在私有沙箱目录下，例如android每个APP特有的/data/data/com.winway.ibitcome/目录下。不要存储到公共的存储区域例如外部storage目录，因为只要其他恶意的app申请了storage的权限就可以对存储在此目录下的keystore文件做操作。
